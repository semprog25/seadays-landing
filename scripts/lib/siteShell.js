                    ...
                    <div class="footer-content">
                        ...
                        <div class="footer-section">
                            ...
                            <ul>
                                ...
                                <li><a href="/meet-saili/">Meet Saili</a></li>
                            </ul>
                        </div>
                        <div class="footer-section">
...
/*
 * Footer HTML always wraps chrome in .container. Pages like /download/ never
 * define that class, so columns sat flush to the viewport. Company pages reuse
 * .container for article width (900px + extra padding), which also leaked in.
 * Own the inner width here so every public footer matches homepage chrome.
 */
footer:has(.footer-shell) > .container,
...
/*
 * Inner pages: starfield/grid are position:fixed; z-index:0/1.
 * A static footer as a body sibling paints UNDER those layers, so visitors
 * see a blank starfield where the footer occupies layout space.
 * Homepage keeps its own fullpage-section layout.
 */
body:not(:has(.fullpage-section)) {
  min-height: 100vh;
  min-height: 100dvh;
  display: flex;
...
body:not(:has(.fullpage-section)) > .content-layer,
body:not(:has(.fullpage-section)) > main.content-layer {
  flex: 1 0 auto;
}

/*
 * Absolute site header does not consume layout space. Blog articles use
 * main.container as the first content block; without top clearance,
 * Back to Blog and breadcrumbs sit under the nav.
 * !important beats later page rules such as .container { padding: 0 20px }.
 * Matches apply-site-shell.js site-shell-page-pad (88px).
 */
body:not(:has(.fullpage-section)) > .content-layer > main.container {
